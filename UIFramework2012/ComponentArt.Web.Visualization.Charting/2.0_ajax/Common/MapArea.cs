...
namespace ComponentArt.Web.Visualization.Charting
{
	...
	internal abstract class MapArea
	{
		...
		{ 
			get
			{
				...
				if(dp != null)
					return dp.OwningSeries.Name + "[" + dp.Index + "]";
				return "";
			}
		}
	...
	internal class MapAreaMatrix : MapArea
	{
		private int[,]	matrix;
		private int		index;
		private Point offsetPosition;

		public MapAreaMatrix(Point offsetPosition,int[,] matrix, int index)
		{
			this.matrix = matrix;
			this.index = index;
		...
		public override MapAreaKind Kind { get { return MapAreaKind.Matrix; } }
		public override int[] Coords { get { return null; } set { } }
		public override bool Contains(Point point)
		{
			Point p = new Point(point.X-offsetPosition.X,point.Y-offsetPosition.Y);
			if(matrix == null)
				return false;

			if(!(0<p.X && p.X<matrix.GetUpperBound(0) && 0<p.Y && p.Y<matrix.GetUpperBound(1)))
				return false;

			int xx = p.X;
			int yy = matrix.GetUpperBound(1) - p.Y;
			int x = matrix[xx,yy];
			if(x == 0)
				return false;
	...
	internal class MapAreaCollection : CollectionWithType
	{
		internal MapAreaCollection() : base(typeof(MapArea),null,
			new Type[] { typeof(MapAreaRectangle), typeof(MapAreaCircle), typeof(MapAreaPolygon) } ) 
		{ }
