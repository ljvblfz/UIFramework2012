...
namespace ComponentArt.Web.UI
{
  ...
  public sealed class ComboBox : WebControl, IPostBackEventHandler
  {
    ...
    internal bool IsMvc = false;
    internal string MvcAjaxUrl = "";
    ...
    protected override void ComponentArtRender(HtmlTextWriter output)
    {
      ...
      output.AddAttribute("id", sComboBoxVarName + "_SelectedText");
      output.AddAttribute("name", sComboBoxVarName + "_SelectedText");
      output.AddAttribute("type", "hidden");
      output.RenderBeginTag(HtmlTextWriterTag.Input);
      output.RenderEndTag();

      // Render client-side-selection-propagating field.
      output.AddAttribute("id", sComboBoxVarName + "_SelectedValue");
      output.AddAttribute("name", sComboBoxVarName + "_SelectedValue");
      output.AddAttribute("type", "hidden");
      output.RenderBeginTag(HtmlTextWriterTag.Input);
      output.RenderEndTag();
      ...
      oStartupSB.Append("['CallbackPrefix','" + Utils.GetResponseUrl(Context).Replace("'", "\\'") + (Context.Request.QueryString.Count > 0 ? "&" : "?") + "Cart_" + sComboBoxVarName + "_Callback=yes'],");
      ...
      if (this.IsMvc)
      {
        oStartupSB.Append("['IsMvc',1],");
        oStartupSB.Append("['MvcAjaxUrl'," + Utils.ConvertStringToJSString(MvcAjaxUrl) + "],");
      }
    ...
    protected override void OnLoad(EventArgs e)
    {
      ...
      if (ScriptManager.GetCurrent(Page) != null)
      {
        ...
        this.RegisterScriptForAtlas("ComponentArt.Web.UI.ComboBox.client_scripts.A573P123.js");
        this.RegisterScriptForAtlas("ComponentArt.Web.UI.ComboBox.client_scripts.A573P456.js");
        if (this.KeyboardEnabled)
        {
          this.RegisterScriptForAtlas("ComponentArt.Web.UI.client_scripts.A573Z388.js");
          this.RegisterScriptForAtlas("ComponentArt.Web.UI.ComboBox.client_scripts.A573P124.js");
        }
      }
    }
