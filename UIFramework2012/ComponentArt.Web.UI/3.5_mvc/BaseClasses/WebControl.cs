...
namespace ComponentArt.Web.UI
{
  ...
  public abstract class WebControl : System.Web.UI.WebControls.WebControl
    ...
    protected override void OnLoad(EventArgs e)
    {
      ...
      sAtlasScript += "window.ComponentArt_Fw35=1;";

      string sFireFoxHookupScript =
        "ComponentArt_FormSubmitHandler(null, true);";

      if (ScriptManager.GetCurrent(Page) != null)
      {
        if (!Page.IsClientScriptBlockRegistered("ComponentArt_Atlas"))
        {
          Page.RegisterClientScriptBlock("ComponentArt_Atlas", "");          
          ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "ComponentArt_Atlas", this.DemarcateClientScript(sAtlasScript), false);
        }

        if (!Page.ClientScript.IsOnSubmitStatementRegistered(this.GetType(), "ComponentArt_FireFoxHookup"))
        {
          Page.ClientScript.RegisterOnSubmitStatement(this.GetType(), "ComponentArt_FireFoxHookup", sFireFoxHookupScript);
        }
      }

      // in sharepoint, force the block into the header
      if (IsInSharePoint())
      {
        Page.Header.Controls.AddAt(0, new LiteralControl(this.DemarcateClientScript(sAtlasScript)));
      }
      ...
      if (_scriptHandlerControls != null)
      {
        ...
        if (_scriptHandlerControls.ToLower() == "percontrol")
        {
          ...
          if (ScriptGenerator.IsScriptControl(sControlName))
          {
            ...
            if (ScriptManager.GetCurrent(Page) != null)
            {
              ...
              {
                ScriptManager.RegisterClientScriptInclude(this, this.GetType(), "ComponentArtScript_Utils", sUtilScriptReference);
                ScriptManager.RegisterClientScriptInclude(this, this.GetType(), "ComponentArtScript_" + sControlName, sControlScriptReference);
              }
              else
              {
                ScriptReference oUtilScriptReference = new ScriptReference(sUtilScriptReference);
                if (!oScriptManager.Scripts.Contains(oUtilScriptReference))
                {
                  oScriptManager.Scripts.Add(oUtilScriptReference);
                }

                ScriptReference oControlScriptReference = new ScriptReference(sControlScriptReference);
                if (!oScriptManager.Scripts.Contains(oControlScriptReference))
                {
                  oScriptManager.Scripts.Add(oControlScriptReference);
                }
              }
            }
            else
        ...
        else
        {
          ...
          if (ScriptManager.GetCurrent(Page) != null)
          {
            ...
            if (oScriptManager.IsInAsyncPostBack)
            {
              ScriptManager.RegisterClientScriptInclude(this, this.GetType(), "ComponentArtScripts", sScriptReference);
            }
            else
            {
              ScriptReference oScriptReference = new ScriptReference(sScriptReference);
    ...
    internal bool IsInSharePoint()
    {
      for (Control oControl = this.Parent; oControl != null; oControl = oControl.Parent)
      {
        if (oControl.GetType().ToString().Contains(".SharePoint."))
        {
          return true;
        }
      }
    ...
    {
      ScriptManager.RegisterStartupScript(this, this.GetType(), sKey, sScript, false);
    ...
    internal string DemarcateClientScript(string script, string title)
    {
      ...
      if (this.IsInUpdatePanel() && this.Context != null && this.Context.Request != null && this.Context.Request.UserAgent != null && this.Context.Request.UserAgent.ToLower().IndexOf("safari") >= 0)
      {
    ...
    protected void WriteStartupScript(HtmlTextWriter output, string sScript)
    {
      ...
      {
        ScriptManager.RegisterStartupScript(this, this.GetType(), this.GetSaneId() + "_" + Guid.NewGuid(), sScript, false);
      }
      else
      {
        output.Write(sScript);
