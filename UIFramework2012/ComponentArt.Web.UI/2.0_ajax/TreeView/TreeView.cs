    ...
    protected override void ComponentArtRender(HtmlTextWriter output)
    {
      ...
      if (this.ClientTarget != ClientTargetLevel.Accessible && !this.IsDownLevel())
      {
        ...
        if(this.EnableViewState)
        {
          ...
          output.AddAttribute("id", sTreeVarName + "_ScrollData");
          output.AddAttribute("name", sTreeVarName + "_ScrollData");
          output.AddAttribute("type", "hidden");
          output.AddAttribute("value", (Page.Request.Form[sTreeVarName + "_ScrollData"] == null ? "0,0" : Page.Request.Form[sTreeVarName + "_ScrollData"]));
          output.RenderBeginTag(HtmlTextWriterTag.Input);
          output.RenderEndTag();
        }
